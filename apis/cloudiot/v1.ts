 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).AuditLogConfig[]} auditLogConfigs The configuration for logging of each type of permission.
Next ID: 4
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).Expr} condition The condition that is associated with this binding.
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {boolean} blocked If a device is blocked, connections or requests from this device will fail.
Can be used to temporarily prevent the device from connecting if, for
example, the sensor is generating bad data and needs maintenance.
* @property {cloudiot(v1).DeviceConfig} config The most recent device configuration, which is eventually sent from
Cloud IoT Core to the device. If not present on creation, the
configuration will be initialized with an empty payload and version value
of `1`. To update this field after creation, use the
`DeviceManager.ModifyCloudToDeviceConfig` method.
* @property {cloudiot(v1).DeviceCredential[]} credentials The credentials used to authenticate this device. To allow credential
...
sent through MQTT.
* @property {string} lastConfigSendTime [Output only] The last time a cloud-to-device config version was sent to
the device.
* @property {cloudiot(v1).Status} lastErrorStatus [Output only] The error message of the most recent error, such as a failure
to publish to Cloud Pub/Sub. &#39;last_error_time&#39; is the timestamp of this
field. If no errors have occurred, this field has an empty message
and the status code 0 == OK. Otherwise, this field is expected to have a
status code other than OK.
* @property {string} lastErrorTime [Output only] The time the most recent error occurred, such as a failure to
publish to Cloud Pub/Sub. This field is the timestamp of
&#39;last_error_status&#39;.
* @property {string} lastEventTime [Output only] The last time a telemetry event was received. Timestamps are
...
* @property {string} lastStateTime [Output only] The last time a state event was received. Timestamps are
periodically collected and written to storage; they may be stale by a few
minutes.
* @property {object} metadata The metadata key-value pairs assigned to the device. This metadata is not
...
* @property {cloudiot(v1).DeviceState} state [Output only] The state most recently received from the device. If no state
...
* @property {string} cloudUpdateTime [Output only] The time at which this configuration version was updated in
Cloud IoT Core. This timestamp is set by the server.
* @property {string} deviceAckTime [Output only] The time at which Cloud IoT Core received the
...
* @property {cloudiot(v1).PublicKeyCredential} publicKey A public key used to verify the signature of JSON Web Tokens (JWTs).
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).RegistryCredential[]} credentials The credentials used to verify the device credentials. No more than 10
...
* @property {cloudiot(v1).StateNotificationConfig} stateNotificationConfig The configuration for notification of new states received from the device.
...
* @property {string} updateTime [Output only] The time at which this state version was updated in Cloud
IoT Core.
*/
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {string} httpEnabledState If enabled, allows devices to use DeviceService via the HTTP protocol.
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).DeviceConfig[]} deviceConfigs The device configuration for the last few versions. Versions are listed
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).DeviceState[]} deviceStates The last few device states. States are listed in descending order of server
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {string} mqttEnabledState If enabled, allows connections using the MQTT protocol. Otherwise, MQTT
...
* @property {cloudiot(v1).Binding[]} bindings Associates a list of `members` to a `role`.
 ...
 * @memberOf! cloudiot(v1)
 * @type object
* @property {cloudiot(v1).Policy} policy REQUIRED: The complete policy to be applied to the `resource`. The size of
