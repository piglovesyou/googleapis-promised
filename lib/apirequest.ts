...
class PromiseWithReq<T> extends Promise<T> {
  req?: object;
}
 ...
 * @return {PromiseWithRequest}  Promise that resolves response data
 */
function createAPIRequest (parameters) {
  ...
  const promise = new PromiseWithReq(function(resolve, reject) {
    ...
    if (missingParams) {
      ...
      reject(new Error('Missing required parameters: ' + missingParams.join(', ')));
